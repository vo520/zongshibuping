...
class Download extends AdminContentBase
{
    ...
    public function add()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            // 处理服务器名称的多语言兼容 - 使用自定义语言包
            if (empty($param['server_name'])) {
                // 如果没有指定服务器名称，根据文件类型设置默认值
                if ($param['file_type'] == 0) {
                    $param['server_name'] = $this->getLocalDownloadText();
                }
            } else if ($param['server_name'] === '本地下载') {
                // 如果传入的是默认中文值，则转换为多语言版本
                $param['server_name'] = $this->getLocalDownloadText();
            }
     ...
     * @return string
     */
    private function getLocalDownloadText()
    {
        // 获取当前语言
        $currentLang = $this->getMyLang();

        // 语言包文件路径
        $langPath = app()->getRootPath() . 'app/lang/pack/' . $currentLang . '.ini';
        ...
        // 读取语言包内容
        $infoContent = file_get_contents($langPath);
        $langstr = "{" . $infoContent . "}";

        // 解析语言包
        $langArr = json_decode($langstr, true);

        // 获取local_download的值
        if (isset($langArr['local_download']['value'])) {
            return $langArr['local_download']['value'];
        }
