...
if (!function_exists("smart_substr")) {
  /**
   * 智能截取字符串，根据当前语言自动选择中英文截取方式
   * @param String $content 要截取的内容
   * @param int $length 截取长度
   * @param float $enMultiplier 英文环境下长度倍数，默认为2
   */
  function smart_substr(String $content, int $length = 30, float $enMultiplier = 2)
  {
      // 获取当前语言
      $currentLang = null;
      ...
      if (function_exists('request') && request()) {
          $currentLang = request()->param("lang");
      }

      // 如果request中没有，则从配置获取
      if (empty($currentLang)) {
          $currentLang = xn_cfg('base.home_lang', 'cn');
      }
      ...
      if (empty($currentLang)) {
          try {
              ...
              if ($defaultLangRecord && !empty($defaultLangRecord['lang'])) {
                  $currentLang = $defaultLangRecord['lang'];
              }
          } catch (\Exception $e) {
              // 数据库查询失败，继续使用默认值
          }
      ...
      // 根据语言选择截取方式
      // 当$currentLang为空或者为中文标识时，使用中文截取方式
      if (empty($currentLang) || in_array(strtolower($currentLang), ['cn', 'zh', 'zh-cn', 'zh_cn'])) {
          return cn_substr($content, $length);
      } else {
          // 英文环境下使用倍数调整长度
          $enLength = (int)($length * $enMultiplier);
          return en_substr($content, $enLength);
      }
  }
}
