...
class AdminContentBase extends Base
{
    ...
    private function menucStat(&$menu, $modelRecords)
    {
        ...
        // 确保使用正确的语言变量
        $currentLang = $this->getMyLang(); // 使用 getMyLang() 方法而不是直接访问 $this->lang

        // 传递当前语言参数给 get_column_down 函数
        $childColumns = get_column_down($id, '', $currentLang);
            ...
            foreach ($modelRecords as $modelRecord) {
                try {
                    if ($modelRecord["reference_model"] == 0) {
                        // 确保表名正确
                        $tableName = $modelRecord["nid"];
                        if (!empty($tableName)) {
                            $count = Db::name($tableName)
                                ->where("column_id", $childColumn["id"])
                    ...
                    \think\facade\Log::error("统计栏目数据出错: " . $e->getMessage());
                }
            }
        }
    ...
    public function synData()
    {
        ...
        // 添加栏目ID筛选条件
        if (!empty($param['columnId'])) {
            array_push($where, ['column_id', '=', $param['columnId']]);
        }
