...
function advertGroupHtml({ gId, gName, isShow, title, linkUrl, imgUrl, imgId }) {
    return [
        ...
        `<input class="foxui-size-small" maxlength="60" placeholder="请输入广告位名称" value="${gName || ''}" name="name"/>`,
...
function advertHtml({ title, linkUrl, imgUrl, imgId }) {
    return [
        ...
        '<label>标题：</label>',
        '<div class="foxui-input-suffix">',
        `<input class="foxui-size-small" maxlength="60" placeholder="请输入标题" value="${title || ''}" name="title"/>`,
        '<i class="foxui-suffix-icon foxui-suffix-count">0/60</i>',
        '</div>',
        '</div>',
